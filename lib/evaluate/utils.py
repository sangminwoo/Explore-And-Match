...
def compute_temporal_iou_batch_paired(pred_windows, gt_windows):
    ...
    >>> spans2 = np.array([[0, 0.3],
                           [0, 1.0]])
    >>> compute_temporal_iou_batch_paired(spans1, spans2)
    np.array([0.6667, 0.5000]
    """
...
if __name__=='__main__':
    ...
    spans2 = np.array([[0, 0.3],
                       [0, 1.0]])
    intersection, union = compute_temporal_iou_batch_cross(spans1, spans2)
    print(intersection)
    print(union)

    intersection = compute_temporal_iou_batch_paired(spans1, spans2)
    print(intersection)